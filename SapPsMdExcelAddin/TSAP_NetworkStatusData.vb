...
Public Class TSAP_NetworkStatusData

    Public aHdrRec As TDataRec
    Public aDataDic As TDataDic
    Public aActivity As String
    ...
    Public Sub New(ByRef pPar As SAPCommon.TStr, ByRef pIntPar As SAPCommon.TStr, ByRef pSAPNetworkPI As SAPNetworkPI, pFunction As String)
        aPar = pPar
        aIntPar = pIntPar
        aFunction = pFunction
        aActivity = ""
        ...
        If pFunction = "GetStatus" Then
            pSAPNetworkPI.getMeta_GetStatus(aParamDic, aStrucDic)
        ElseIf pFunction = "SetStatus" Then
            pSAPNetworkPI.getMeta_SetStatus(aParamDic, aStrucDic)
        End If
    End Sub
    ...
    Public Function fillHeader(pData As TData) As Boolean
        ...
        For Each aKvb In aPar.getData()
            ...
            For s As Integer = 0 To aStrucName.Length - 1
                ...
                End If
                If (aNewTStrRec.Strucname = "I_ACTIVITY_SYSTEM_STATUS" Or aNewTStrRec.Strucname = "I_ACTIVITY_USER_STATUS") And
                    aNewTStrRec.Fieldname = "ACTIVITY" Then
                    aActivity = aNewTStrRec.formated()
                End If
            Next
        Next
    ...
    Public Sub dumpData()
        Dim dumpDt As String = If(aIntPar.value("NETW_DBG", "DUMPDATA") <> "", aIntPar.value("NETW_DBG", "DUMPDATA"), "")
        If dumpDt <> "" Then
            Dim aDWS As Excel.Worksheet
            Dim aWB As Excel.Workbook
            Dim aRange As Excel.Range
            ...
            Catch Exc As System.Exception
                ...
                MsgBox("No " & dumpDt & " Sheet in current workbook. Check the NETW_DBG-DUMPDATA Parameter",
